...
- 새로운 위치 포즈 추가해서 정해진 위치에 place까지 완료

- yolo모델로 판별한 박스의 정확도가 0.9 초과면 양품, 0.9 이하면 불량품으로 구별하도록 설정
"""
...
def pixel_to_robot(cx, cy, frame_w, frame_h):
    ...
    dx = (cx - frame_w / 2) * -(0.001)
    dy = (cy - frame_h / 2) * 0.067
...
def detect_object(model, frame):
    ...
    for r in results:
        for box in r.boxes:
            ...
            name = r.names[cls]
            accuracy = conf
            print(f"🧩 감지됨: {name} | conf={conf:.2f} | box=({x1:.1f}, {y1:.1f}, {x2:.1f}, {y2:.1f})")
...
def main():
    ...
    if detected:
        ...
        #PLACE
        mc.send_coords(POSE_PLACE, DEFAULT_SPEED, 0)
        time.sleep(2)

...
POSE_HOME = [-200.6, 29.8, 378.8, -171.43, 19.88, 0.87]
...
def detect_object(model, frame):
    ...
    if len(boxes) == 0:
        return frame_vis, None, None, None, None, None
    ...
    cv2.putText(frame_vis, f"{name} ({conf:.2f}) | {angle:.1f}°", (int(x1), int(y1)-10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,0), 2)

    print(f"🧩 감지됨: {name} | conf={conf:.2f} | angle={angle:.1f}° | center=({cx},{cy})")
    return frame_vis, (cx, cy), angle, conf, frame.shape[1], frame.shape[0]
...
def main():
    ...
    while not stop_event.is_set():
        ...
        if frame is None:
            continue
        frame_vis, result, angle, conf, fw, fh = detect_object(model, frame)
    ...
    if frame is not None:
        re_frame, _, new_angle, _, _, _ = detect_object(model, frame)

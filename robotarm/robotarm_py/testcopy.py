...
POSES = {
    ...
    "Place": [158.0, -59.2, 431.5, -142.63, 1.04, -79.0],  # 예: 내려둘 위치
    "Box1": [291.3, 210.0, 171.8, -172.57, -1.46, -87.15],  # 예: 내려둘 위치
    "Box2": [333.4, 11.7, 170.9, -175.19, -0.08, -89.53],  # 예: 내려둘 위치
    "Box3": [319.9, -169.5, 166.6, -172.32, -2.86, -87.15],  # 예: 내려둘 위치
}
...
def move_to(mc, name, speed=DEFAULT_SPEED):
    ...
    # 명령을 쓴다고 보면 됨.
    mc.send_coords(POSES[name], speed)
    print(f"➡️ Move: {name}")
...
def main():
        ...
        if in_roi:
            if roi_detect_start is None:
                ...
                if elapsed >= DETECT_HOLD_TIME:
                    ...
                    # -----------------------------
                    try:
                        # 저장된 변환행렬 불러오기 (예: /home/vboxuser/robotarm/handeye_result.yaml)
                        fs_handeye = cv2.FileStorage("/home/vboxuser/robotarm/handeye_result.yaml", cv2.FILE_STORAGE_READ)
                        R_mat = fs_handeye.getNode("R").mat()
                        t_vec = fs_handeye.getNode("t").mat()
                        fs_handeye.release()

                        if R_mat is None or t_vec is None:
                            raise ValueError("handeye_result.yaml에 R 또는 t 데이터가 없습니다.")

                        # 카메라 좌표 (단위: mm)
                        cam_point = np.array([[Xc_mm], [Yc_mm], [Zc_mm]])  # 검출된 픽셀 기반 좌표

                        # Hand-Eye 변환: 로봇 좌표 = R * 카메라좌표 + t
                        robot_point = R_mat @ cam_point + t_vec
                        ...
                        print(f"✅ 변환된 좌표 (Hand-Eye 적용): X={coord_data['x']:.2f}, Y={coord_data['y']:.2f}, Z={coord_data['z']:.2f}")

                    except Exception as e:
                        print(f"❌ Hand-Eye 변환 실패: {e}")
    ...
    # mc.send_coords(POSES["Place"], args.speed)
    # time.sleep(3)
    # print("✅ 플레이스 기본 위치 도달 완료")
    
    # #박스1
    # print("플레이스 기본 위치로 이동 중...")
    # mc.send_coords(POSES["Box1"], args.speed)
    # time.sleep(3)
    # print("✅ 플레이스 기본 위치 도달 완료")

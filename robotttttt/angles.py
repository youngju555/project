...
[개선 버전 v5.11 - YOLO + 회전각 보정 + 다중 클래스 분류]

📌 v5.10 대비 핵심 변경점
----------------------------------------------------
1. (로직) YOLO 스레드가 좌표, 클래스 ID와 더불어 '회전 각도(angle)'도 계산
   - YOLO가 Box를 찾으면, 해당 영역을 잘라(ROI) OpenCV로 각도를 계산
   - [수정] g_target_object: "angle" 값 추가 저장

2. (로직) 로봇 스레드가 전달받은 angle을 RZ(Yaw) 자세에 더하여 보정
   - [수정] mc.send_coords 호출 시 RZ 값이 (PICK_RZ + angle)로 변경됨
   - 큐브가 삐딱하게 있어도, 로봇이 손목을 회전시켜 정렬 후 집음
"""
...
def robot_control_thread(stop_event, mc, dry_run):
    ...
    while not stop_event.is_set():
        ...
        if current_task:
            ...
            FIXED_PICK_Z = 278.0  # (참고: v8.0 코드는 290mm 근처였음, 필요시 308.0로 수정)
            APPROACH_HEIGHT = 40.0
            PICK_RX, PICK_RY, PICK_RZ = -175.33, 8.65, 86.68 # 기본 RZ(Yaw) 자세
            
            # [!!! v5.11 수정 !!!] RZ(Yaw) 값에 계산된 각도 보정
            # OpenCV 각도(반시계가 +)와 로봇 RZ(반시계가 +)가 일치한다고 가정
            # 만약 방향이 반대면 'angle' 대신 '-angle'을 사용
            corrected_rz = PICK_RZ + angle 
            print(f"  ↳ RZ 보정: {corrected_rz:.2f} (기본: {PICK_RZ} + 각도: {angle:.2f})")
            ...
            if not dry_run and mc is not None:
                ...
                mc.set_gripper_value(50, 80, 1) # 그리퍼 열기 (v5.10 기준)
                time.sleep(1)
                
                # [!!! v5.11 수정 !!!] corrected_rz 적용
                ...
                mc.set_gripper_state(1, 80) # 그리퍼 닫기 (v5.10 기준)
                time.sleep(1.5)
                
                # [!!! v5.11 수정 !!!] corrected_rz 적용 (잡고 들어올릴 때 자세 유지)
                mc.send_coords([pick_x, pick_y, z_grasp + 80, PICK_RX, PICK_RY, corrected_rz], 25, 0) # mode=0 (각도)
                time.sleep(2)
    ...
    try:
        ...
        if not args.dry_run:
            try:
                ...
                mc.set_gripper_state(0, 80) # [!!! v5.11 수정 !!!] 닫힘(1) -> 열림(0)으로 변경 (로직 일관성)
                time.sleep(1)
            except Exception as e:
                print(f"❌ 로봇 연결 실패: {e}")
                mc = None
